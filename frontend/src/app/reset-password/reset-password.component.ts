      ...
      this.accountService.resetPassword(email).subscribe(
        ...
        (error: HttpErrorResponse) => {
          ...
          if (errorMsg === 'emailNotFound') {
            this.alertService.showAlert('There is no account for this email. Please verify the email.', AlertType.DANGER);
          }
          if (errorMsg !== 'emailNotFound') {
            this.alertService.showAlert('Some error occured. Please try again.', AlertType.DANGER);
          }
          this.loadingService.isLoading.next(false);
        }
      )
